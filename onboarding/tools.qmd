...
In this book, we'll be using R and the **RStudio IDE**. You'll need to download each from the internet to install and use on your local workstation.
...
>
> — Ihaka & Gentleman (1996) see @ihaka1996r, p. 2
...
There are two pieces of software you'll need to download. One is R itself and the other is the *Community Edition* of the **RStudio IDE** IDE. Luckily, you can use both of these in Windows, macOS, and in various distributions of Linux. Depending on your OS there are preferred ways to install both. While we do need both pieces of software installed, the order of installation is not important.
...
2. Once there, look for the macOS installation file of **RStudio Desktop Community Edition**. If browsing from a Mac, the site will present you with a large download button that leads to the latest stable Mac release of **RStudio IDE**.
...
2. Look for the Windows install of **RStudio IDE Community Edition**. A large download button for the Windows version of **RStudio IDE** will appear (it leads to the latest stable Windows release).
...
**RStudio IDE** is an integrated development environment (IDE) that makes it easy to do work with R (we'll simply refer to this IDE as **RStudio** throughout this book). **RStudio** and R itself both need to be installed separately (the order of installation does not matter). Detailed instructions for the installation of both (for Windows and macOS) are available in the *Appendix*.
...
![The **RStudio IDE** environment with an **Quarto** file in focus in top-left (the source pane) and the R console occupying the bottom-left.](appendix-figures/82-starting-with-r/figure-rstudio-desktop-env.png){#fig:rstudio-desktop-env}
...
Indeed, a little can go a long way, so, while on the topic of the small things, it's never too early to learn about a few useful keyboard shortcuts. These are the ones that you'll use again and again.
  ...
  4. <kbd>Command</kbd> + <kbd>Option</kbd> + <kbd>I</kbd> (<kbd>Control</kbd> + <kbd>Alt</kbd> + <kbd>I</kbd> on Windows) — creates an empty R code chunk in an **Quarto** document. As this is one of the key things you'll be doing, this shortcut does save a lot of time.
