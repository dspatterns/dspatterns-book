...
An online version of this book is available at `<insert link here>`.
...
Want to know what goes into the sausage? Here is an overview of our book tech stack.
...
The combination of {{< var gha.docs >}} and {{< var web.host >}} means that the book is automatically updated after every commit to the `main` branch on GitHub.[^longnote]

[^longnote]: Every GitHub Action workflow has a link to view logs for workflow runs.
...
- Finally, we introduced {{< var renv.pkg >}} to our stack, following {{< var renv.docs >}}.[^book-4] This ensured that we were both working on and building the book locally with the exact same versions for all of our R packages.
...
- Running `renv::snapshot()` if we added any new packages so that we updated {{< var renv.lock >}}
