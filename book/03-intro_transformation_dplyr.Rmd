...
```{r environment-tab, echo = FALSE, fig.cap='(ref:environment-tab)'}
knitr::include_graphics("figures/chapter_03/03-01-figure-environment-tab.png")
```
...
"#B The **dspatterns** package is this book's namesake package. It has the datasets we need for all of the examples."
))`
...
c(
"#A So long as we have the **dspatterns** package loaded in (with ~~library(dspatterns)~~), we have access to the ~~sw~~ dataset. Executing ~~sw~~ (a *tibble*) prints it and shows us some of its values."
))`
```{r sw-tibble-print}
sw  #A
```
...
1. One or more `library()` statements should ideally come first to load in package functions. For this lesson, the statements could be `library(tidyverse)` and `library(dspatterns)` (to use datasets and functions particular to this book).
