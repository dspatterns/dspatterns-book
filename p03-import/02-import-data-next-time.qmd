...
On point 3, recall that the country names were suspect? Looking at the set used in the `col_vals_in_set()` statement, we can see that there are a few two-letter country names in the mix (that's not right). We'll have to change the way we check that column. Maybe the most prudent way to check for this error happening in the future is to check for a string length of at least four (this could guard against three-letter country codes erroneously entered in this column).
...
There are issues with the `first_login` datetime values (16 units failing), the values within `country` (47 failures), and the `country` and `acquistion` columns have `NA`s (17 and 9, respectively).
