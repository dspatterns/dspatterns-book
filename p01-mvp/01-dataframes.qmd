...
```{r}
#| label: tibble-na-col
#| caption: Using a single-length vector with an `NA` value in `tibble()`.
#| paged.print: false
...
```
1. Using a single `NA` (and nothing else) gives us a certain type of `NA`: a `logical` `NA` (yes, there are different types).
