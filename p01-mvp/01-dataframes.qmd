...
Creating your own tabular data can be really useful for sharing (especially when you need to create a particular one for debugging something) and other purposes. To that end, we'll learn how to make our own tibbles from scratch. Although we customarily get our data from other sources (e.g., CSV files, database tables, **Excel** files, etc.), there are a few good reasons for wanting to handcraft our own tibble objects:
