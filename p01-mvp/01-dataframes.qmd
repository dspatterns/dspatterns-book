...
Tables come in all shapes and sizes. Some are very small, filling maybe a screen or two with data; some are in between, requiring querying and summarization to really make sense of the data; and some are huge, necessitating more efficient tools for transforming, querying, and summarizing the data. Here, we'll start small and learn how to explore tables of the small variety. In **R**, tabular data is often stored in data structures known as data frames. It's part of *base R* and you don't need any extra packages to use data frames.
...
Let's look at some data from `episodes` dataset in the **dspatterns** package. It serves as a highly quantitative episode guide. Before we start, we'll load in the necessary packages, namely the **tidyverse** and **dspatterns** packages (the latter will load the **bakeoff** package).
...
It provides a lot of information, but, it doesn't overload the console with multiple screenfuls of information. Because the **tidyverse** packages are loaded, the tibble version of the dataset is what's given during printing. At any rate we can see some of the data (10 rows) in the first few columns of the `episodes` dataset.

If you wanted few rows, you could use the `head()` function like this:
...
and the above is great if you have extremely limited space, or, if you are dealing with data frames (which will print out many, many more rows).

Something else that's very useful during the *exploration* phase of  
...
Creating your own tabular data can be really useful for sharing with another person (especially when you need to create a particular one for debugging something).
