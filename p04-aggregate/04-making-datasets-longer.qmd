...
Now that we have long data in the object `illness_long`, let's make a **ggplot** plot. We'll use `facet_wrap()` to facet the plots by the specific test.
...
This doesn't look good when trying to display too many tests. We're going to have to take decisive action to make this all look better.
