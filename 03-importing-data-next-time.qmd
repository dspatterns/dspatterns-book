# Importing data for the next time
...
- `stickers` raw to clean using qmd + experience from (use `knitr::purl()` to get .R script from .qmd, rewrite/refactor to get 'transformation' chunk -> we can `source()` that in, it will contain the data reading, cleaning, transformation, etc.)
...
- come up with pointblank validation and create rules using your domain knowledge (you have to read up on the research around the penguin subjects)
