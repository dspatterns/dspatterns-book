...
We must remember to load the packages we need for this chapter! This will comprise three `library()` statements, one each for: tidyverse (which loads a lot of *Tidyverse* packages, including **dplyr** and **ggplot**), **lubridate** (not loaded by `library(tidyverse)`), and **dspatterns** (to get access to our datasets).
...
```{r plot-nycweather-asis, fig.cap='(ref:plot-nycweather-asis)', eval=FALSE}
