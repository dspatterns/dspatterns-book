...
As far as packages requirements, we will need **bakeoff**, **tidyverse**, and **pointblank**.
...
## Understand
  ...
  - tables + viz (distinct / count / geom_col)
  - from exploratory plots (confirms or disproves your assumptions about the data)

To get an even greater understanding of our data, we might want to explore it. Let's explore the `bakers` dataset from the **bakeoff** package. This dataset provides a record for every contestant from every series of the show.
...
This is exactly what we need and it's now plain to see that the most common first name is Kate (with three *different* contestants having that name).

Was there an awkward series where two contestants shared the same first name? We can find that out. Let's do a count of baker by series (were there any series where two people shared the same name?)

```{r, paged.print=FALSE}
bakers %>% count(series, baker_first, sort = TRUE)
```

It seems like this is not the case. All `n` values are `1` and the total count here is the same as before (`120`). This follows the original order of the table (series increasing). Interestingly, the first series started with only 10 but it became at least bakers dozen's worth later on. 

Can we figure out if any bakers are from the same hometown? How about from the same hometown and the same series?
    
```{r, paged.print=FALSE}
bakers %>% count(hometown, series, sort = TRUE)
```
    
We can extend this line of thinking to get a count of contestants per episode across all series?
