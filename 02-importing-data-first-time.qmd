...
After making the `slice()`, we should again view the tibble to see if the result is satisfying:

```{r, paged.print=FALSE}
stickers
```

Much better since the unwanted row is now gone! We have to keep in mind, though, that printing out a tibble only gives us a tiny preview of the data. We can look at the tail of the data with `tail()` to verify that the end of the table is what we might expect.

```{r, paged.print=FALSE}
tail(stickers)
```

Now let's clean these terrifying names with **janitor**'s world-beating `clean_names()` function:

```{r}
stickers <- clean_names(stickers)
```

Now, let's have a look at just revised set of column names in the revised `stickers`. We can easily do this with `colnames()` and here it is:

```{r, paged.print=FALSE}
colnames(stickers)
```

These column names have been cleaned up by transforming them to `snake_case`, which are words separated by the underscore (`_`) character.
...
These column names present a bit better now, however, we still have an outrageously long column name (`absolutenumberofstickersgiven_conditions1or3_outof12_conditions2or4_outof30`) and having that is not at all great for readability. Let's rename that and, while we're at it, rename quite a few of other columns where the names are verbose, redundant, or confusing. We'll do this with **dplyr**'s `rename()` function. Like this:
...
It looks *almost* right. The incident with the bad raw early on has forced most of the columns to be of the `character` type. We can see here that every column except for `subject` should be `numeric`. Let's make a correction to `stickers` that changes many column types all at once.

```{r, paged.print=FALSE}
stickers <-
  stickers %>%
  mutate(across(.cols = 2:last_col(), .fns = as.numeric))
```

Using `mutate()` and `across()` is tricky at first but very worthwhile since we can apply the same transformation to multiple columns. Within `across()`, the important components are a selection of columns (in `.cols`) and the function to apply (in `.fns`). For the latter, we want the columns of our choosing to be numeric so we'll supply `as.numeric` here. To choose the columns, we can specify them with a range using the `:` style. We'll start at `2` and end at the last column. The **dplyr** package has a helper function `last_col()` that's perfect for this, so, the column selection becomes `2:last_col()`.

Let's now have a `glimpse()` at the changed `stickers` table and make certain that the transformation worked out:
...
Once the data dictionary has been produced for `stickers`, we can look at it in the Viewer by printing it out.
