...
When working in R long enough, you'll encounter list objects again and again. You may bump into factors every now are again but try to side-step their use. However, these are very important and useful data structures and we ought to fully understand them. This section goes over lists, which we've seen before, in much more detail. We'll understand how to better create lists, how to access data from them, how to combine lists together, and how to go to and from lists and data frames. Factors will be introduced in the second part of this section. They shouldn't be something to be avoided as they are genuinely useful, especially in the context of plotting data with **ggplot**.

We'll use a dataset called `german_cities` (available in the **dspatterns** package) to work through examples that involve factors, eventually getting to nicer and nicer plots. The functions that we'll use throughout this section will be an even mix of base R functions and functions that are available in the *Tidyverse* packages **forcats** and **ggplot**.
...
"Creating a three-element, named list object."
)`
...
`r dspatterns::code_hints(
"Examining the structure of a list object with ~~str()~~."
)`
...
`r dspatterns::code_hints(
"Getting the names of the list components with ~~names()~~."
)`
```{r names-in-a-list}
names(a_list)
```
...
`r dspatterns::code_hints(
"Getting the first element of ~~a_list~~ with ~~[ ]~~."
)`
...
`r dspatterns::code_hints(
"Creating a three-element, unnamed list object."
)`
...
`r dspatterns::code_hints(
"Examining the structure of the ~~b_list~~ object with ~~str()~~."
)`
...
`r dspatterns::code_hints(
"Giving the unnamed list (~~b_list~~) some names."
)`
...
`r dspatterns::code_hints(
"Modifying the tibble component of ~~b_list~~ requires accessing the ~~b_list$b~~ component."
)`
...
`r dspatterns::code_hints(
"Combining three named lists into a larger list with ~~c()~~ (reproducing the ~~a_list~~ object)."
)`
...
`r dspatterns::code_hints(
"Removing the second element of the ~~a_list~~ object using ~~NULL~~."
)`
...
`r dspatterns::code_hints(
"Removing the first element of the ~~a_list~~ object using ~~NULL~~ (this time with an index value)."
)`
...
`r dspatterns::code_hints(
"A named list that is suitable for transformation to a data frame or tibble."
)`
...
`r dspatterns::code_hints(
"Transforming the ~~list_df~~ list into a data frame with ~~as.data.frame()~~."
)`
...
`r dspatterns::code_hints(
"Transforming the ~~list_df~~ list into a tibble with ~~as_tibble()~~."
)`
...
`r dspatterns::code_hints(
"Transforming the tibble into a list with ~~as.list()~~."
)`
...
`r dspatterns::code_hints(
"Example of a function (~~get_descriptive_stats()~~) that returns a list object."
)`
...
`r dspatterns::code_hints(
"Example of a function (~~add_percentiles()~~) that takes a list object as input and returns an augmented version of the input list."
)`
```{r add-percentiles}
...
The *Tidyverse* package **forcats** will be used here for handling factor values. We have an interesting dataset available in the **dspatterns** package called `german_cities.` It's a bit like the `us_cities` dataset we used previously except it's smaller (only 79 rows) and has two factor columns (`name` and `state`). A dataset like this may be used by statisticians to analyze population trends, or, it might be used in combination with other domain-specific data (e.g., in advertising/marketing, scientific studies, etc.). Here is a printout of the `german_cities` tibble.

`r dspatterns::code_hints(
"Printing out the ~~german_cities~~ dataset, which has two factor columns."
)`
...
`r dspatterns::code_hints(
"Printing out the unique values from the ~~state~~ column of ~~german_cities~~ (obtained as a vector through ~~$~~ indexing)."
)`
```{r unique-state-fct}
...
`r dspatterns::code_hints(
"Getting all the levels for the ~~state_fct~~ factor using the ~~levels()~~ function."
)`
```{r state-fct-levels}
levels(state_fct)
```
...
`r dspatterns::code_hints(
"Getting the integer values for the factor levels in ~~state_fct~~ with ~~as.integer()~~."
)`
...
`r dspatterns::code_hints(
"Understanding the frequency of factor levels with the **forcats** ~~fct_count()~~ function."
)`
...
`r dspatterns::code_hints(
"Filtering the cities in ~~german_cities~~ to those in the state of Bayern."
)`
...
`r dspatterns::code_hints(
"Dropping unused factor levels in the ~~name~~ and ~~state~~ ~~\"factor\"~~ columns with ~~fct_drop()~~."
)`
```{r mutate-fct-drop}
german_cities_bayern <- 
  german_cities_bayern |> 
...
We checked our work by using the `levels()` function for manual inspection of the levels. We now get an ordering that is no longer alphanumeric. This is success! We'll really see it, though, when the bar plot is regenerated. We can re-run the plotting code and it'll give us a radically improved plot.

`r dspatterns::code_hints(
"Regenerating the barplot of the most populous German cities in the state of Bayern (this is version 2)."
)`
```{r}
#| label: fig-barplot-bayern-2
#| fig.cap: "An improvement on the bar plot of large cities in Bayern, Germany. Reordering factor levels in the name variable leads to a coherent ordering of cities by population."

ggplot(data = german_cities_bayern, aes(y = name, x = pop_2015)) +
  geom_bar(stat = "identity")
```

Having the bars arranged in this way really helps. We can tell, in an instant, that Ingolstadt has a greater population than Würzburg. What if you wanted the bars in the reverse order? In that case, the `fct_rev()` function is what's needed. Let's use the function, inspect the factor levels with `levels()`, and re-plot as before.

`r dspatterns::code_hints(
"Reversing the order of factor levels in the ~~name~~ variable with the ~~fct_rev()~~ function."
)`
```{r fct-rev}
...
`r dspatterns::code_hints(
"Regenerating the barplot of the most populous German cities in the state of Bayern (this is version 3)."
)`
```{r}
#| label: fig-barplot-bayern-3
#| fig.cap: "The reversed order of bars in this plot was accomplished by reversing the factor levels of the name variable with `fct_rev()`."
...
`r dspatterns::code_hints(
"Recoding factor levels in name with the ~~fct_recode()~~ function."
)`
```{r fct-recode}
german_cities_bayern$name <- 
  fct_recode(
...
Again, we need to plot this data to really believe that this use of `fct_recode()` will change the plot labels.

`r dspatterns::code_hints(
"Regenerating the barplot of the most populous German cities in the state of Bayern (this is version 4)."
)`
```{r}
#| label: fig-barplot-bayern-4
#| fig.cap: "Thanks to selectively recoding factor values with `fct_recode()`, some of the city names have been slightly altered. The rest of the plot remains unchanged."
...
Let's take the same dataset, `german_cities`, and get a bar plot that indicates how many cities are in each state. The use of `geom_bar()` is now different than what was used earlier. This time we will use the `stat = "count"` option but, since that's the default for `geom_bar()`, we'll just omit that altogether. Have a look at the **ggplot** code and the resulting plot.

`r dspatterns::code_hints(
"Creating a bar plot that is based on counts of cities in each state, which is mapped to the ~~y~~ aesthetic."
)`
```{r}
#| label: fig-barplot-count
#| fig.cap: "A bar plot that shows the number of cities in the dataset that are part of each state. We need to order the bars, however, to make this a more effective visualization (it's hard to parse at present)."
...
The plot is somewhat unsatisfactory as a visualization since the bars (corresponding to counts of cities in each state) are not ordered by their lengths. The **forcats** package offers a very useful function that assigns frequency values to factor levels: `fct_infreq()`. Let's use `fct_infreq()` in a `mutate()` statement along with `fct_rev()` as a way to reverse the levels, just as we've done before. In the next code listing the dataset is mutated with those factor adjustments and the table is then introduced to `ggplot()` via the `|>`.

`r dspatterns::code_hints(
"Improving on the city count bar plot through use of the ~~fct_infreq()~~ and ~~fct_rev()~~ functions."
)`
```{r}
#| label: fig-fct-infreq
#| fig.cap: "An improved bar plot that made use of the `fct_infreq()` function (to assign levels according to frequency) and, following that, the `fct_rev()` function (to reverse the order of levels)."
...
This plot is a great improvement over the previous one, proving yet again that factors, and the careful manipulation of them, are useful for controlling plot presentation.
...
`r dspatterns::code_hints(
"Using the ~~fct_inseq()~~ function to reorder a factor's levels by its numeric values."
)`
```{r fct-inseq}
...
`r dspatterns::code_hints(
"Further refinement of the city count bar plot by incorporating the ~~fct_lump_n()~~ function (just before reversing levels)."
)`
```{r}
#| label: fig-fct-lump-n
#| fig.cap: "This bar plot of states and the count of cities within them now has an 'Other' bar that represents the count of cities in all other states not in the highlighted five."
