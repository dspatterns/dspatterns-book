...
![A **DT** table using the default options as it appears in an **Quarto** document.](img/tables-01-figure-dt_pitchfork_1.png){#fig-dt-pitchfork-1}
...
![A **DT** table using the default options as it appears in a browser (or the **Quarto** *Preview* window.](img/tables-02-figure-dt_pitchfork_1b.png){#fig-dt-pitchfork-1b}
...
![An improved **DT** table that has functional links in the rightmost column.](img/tables-03-figure-dt_pitchfork_2.png){#fig-dt-pitchfork-2}

![A web page that appeared before my eyes (in my default browser) when I clicked on a link in the **DT** table (row 7, it was).](img/tables-04-figure-dt_pitchfork_2b.png){#fig-dt-pitchfork-2b}
...
![A **DT** table with *Bootstrap 4* styling and other refinements.](img/tables-05-figure-dt_pitchfork_3.png){#fig-dt-pitchfork-3}
...
![The **DT** table with modified column labels.](img/tables-06-figure-dt_pitchfork_4.png){#fig-dt-pitchfork-4}
...
![Filtering controls in the **DT** table for each column.](img/tables-07-figure-dt_pitchfork_5.png){#fig-dt-pitchfork-5}
...
![A **DT** table with the complete set of modifications, the last being the addition of a panel of buttons for downloading, copying, and printing data.](img/tables-08-figure-dt_pitchfork_6.png){#fig-dt-pitchfork-6}
...
![This is the `exibble` dataset as a **gt** table.](img/tables-09-figure-gt_exibble_1.png){#fig-gt-exibble-1}
...
![The parts of a **gt** table.](img/tables-10-figure-parts_of_a_gt_table.png){#fig-parts-of-a-gt-table}
...
![The `exibble` **gt** table with a table header added (via the `tab_header()` function).](img/tables-11-figure-gt_exibble_2.png){#fig-gt-exibble-2}
...
![The `exibble` **gt** table with a both a table header and a table footer (containing two source notes provided by two uses of the `tab_source_note()` function).](img/tables-12-figure-gt_exibble_3.png){#fig-gt-exibble-3}
...
![The footer of the `exibble` **gt** table now has a footnote that's associated with a specific body cell. With multiple calls of `tab_footnote()`, several footnotes can be added.](img/tables-13-figure-gt_exibble_4.png){#fig-gt-exibble-4}
...
![Two different footnotes, two different locations: the secret to this is using those location helper functions for targeting.](img/tables-14-figure-gt_exibble_5.png){#fig-gt-exibble-5}
...
![A **gt** presentation table made with the `oceaniapops` data table by just using `gt()`. It's nice but it could be made nicer.](img/tables-15-figure-gt_oceaniapops_0.png){#fig-gt-oceaniapops-0}
...
![A much-improved version of the previous **gt** table! It now contains row labels in the stub area and row group labels for each of the regions of Oceania. Superb!](img/tables-16-figure-gt_oceaniapops_1.png){#fig-gt-oceaniapops-1}
...
![The top portion of the **gt** table we've been working on (for the Oceania project). Now the column labels have a spanner column label above.](img/tables-17-figure-gt_oceaniapops_2.png){#fig-gt-oceaniapops-2}
...
![The **gt** table's numeric values are way more readable now thanks to the numeric formatting and column alignment (to the right).](img/tables-18-figure-gt_oceaniapops_3.png){#fig-gt-oceaniapops-3}
...
![The now-wider `oceaniapops` **gt** table now has a title and it looks great when styled with Markdown text.](img/tables-19-figure-gt_oceaniapops_4.png){#fig-gt-oceaniapops-4}
...
![The addition of footnotes and a source note adds more useful information to the table. Because we used Markdown, the source note even has a link to a web page.](img/tables-20-figure-gt_oceaniapops_5.png){#fig-gt-oceaniapops-5}
...
![A **gt** table can be styled with the `tab_style()` function. Here, multiple locations' cells are styled with bold text.](img/tables-21-figure-gt_oceaniapops_6.png){#fig-gt-oceaniapops-6}
...
![The finalized `oceaniapops` **gt** table. It has summary rows (per group and for the whole table).](img/tables-22-figure-gt_oceaniapops_7.png){#fig-gt-oceaniapops-7}
