...
In one of our very first code listings in this Appendix, we assigned a character string to an object. We did this by surrounding the text within double quotes (`" "`).

`r dspatterns::code_hints(
"Assigning a character string to ~~one_word~~.",
c(
"L.1 We need quotes to distinguish character strings from variables."
))`
```{r one-word}
one_word <- "hello"
one_word
...
`r dspatterns::code_hints(
"Creating a character vector with three elements."
)`
```{r three-words}
...
`r dspatterns::code_hints(
"Combining two character vectors with the ~~c()~ function."
)`
```{r words-and-days}
...
`r dspatterns::code_hints(
"Combining a character vector with a numeric vector."
)`
```{r words-and-numbers}
...
`r dspatterns::code_hints(
"How combining elements of different types results in coercion."
)`
...
`r dspatterns::code_hints(
"Using the ~~stringsAsFactors~~ option with ~~data.frame()~~."
)`
...
`r dspatterns::code_hints(
"Using the ~~formatC()~~ function to get very particular output character strings for three decimal numbers.",
c(
"L.1 The values have varying numbers of decimal digits (3, 2, and 4 digits).",
"L.5 Setting digits to ~~3~~ means that we need exactly three digits after the decimal mark. Rounding will occur.",
"L.6 The format designation is pretty important here. Using ~~\"f\"~~ will give us numbers in the ~~xxx.yyy~~ format.",
"L.7 The choice of decimal mark is usually decided by your system's locale. It can be an explicit character though and, here, the period character is chosen for ~~decimal.mark~~.",
"L.8 The ~~drop0trailing~~ option will discard any trailing zeros after the decimal place if this is set to ~~TRUE~~. The default is ~~FALSE~~."
))`
...
`r dspatterns::code_hints(
"Using ~~formatC()~~ in a ~~mutate()~~ call to format sequential numeric values to index strings that are left-padded with zeros (to a specified width).",
c(
"L.2 The ~~tibble()~~ statement generates a 1200 row tibble object.",
"L.3 The ~~mutate()~~ call will write over the index column with the output values.",
"L.5 The ~~digits~~ value being ~~4~~, this time, indicates total digits shown (there won't be decimal places because of the format specification we are providing).",
"L.6 The format of ~~\"d\"~~ indicates that we want integers (no decimals).",
"L.7 The flag is a format modifier (and there are several that ~~formatC()~~ understands); a flag of ~~\"0\"~~ is understood as a directive to pad with zeros on the left.",
"L.9 Instead of using **dplyr**'s ~~mutate()~~ to ~~paste0()~~ strings together, the **tidyr** ~~unite()~~ function is used here.",
"L.10 Reverse sorting on ~~combined~~ is done here to prove that the alphanumeric sorting works now that the numeric portion is padded with zeros."
))`
...
`r dspatterns::code_hints(
"Using the ~~toupper()~~ and ~~tolower()~~ functions to transform characters to upper case and lower case."
)`
```{r toupper-tolower}
...
`r dspatterns::code_hints(
"Using ~~abbreviate()~~ to make unique abbreviations for longer strings.",
c(
"L.1 This is the character vector with terms to abbreviate; notice that some of these are pretty close in spelling (and there's a duplicated entry in there as well).",
"L.3 All strings in the ~~terms~~ vector are abbreviated with a minimum length of 3 characters with ~~minlength = 3~~."
))`
```{r abbreviate-terms}
...
`r dspatterns::code_hints(
"Using ~~trimws()~~ to remove any leading and trailing whitespace in our truncated strings.",
c(
"L.1 The character vector of truncated strings is piped to ~~trimws()~~; the default options (trimming whitespace on the *left* and *right* and the function's definition of whitespace) are usually always adequate for this type of string operation.",
"L.3 Again, we are just printing the first five elements of (the newly modified) ~~resto_reviews_trunc~~ vector."
))`
...
`r dspatterns::code_hints(
"Using ~~strtrim()~~ to truncate strings, much like ~~substr()~~.",
c(
"L.1 Every output element of the ~~substr()~~ expression is compared to that of the ~~strtrim()~~ expression with ~~all.equal()~~. If they all match (and they do here), the result is ~~TRUE~~."
))`
...
It's good to have a playground for experimenting with regexes and for that, the *RegExplain* gadget for **RStudio** (developed by Garrick Aden-Buie) is really good. It allows us to quickly experiment with our regex patterns. It's included as part of the **dspatterns** package for convenience; all we need to do is run `dspatterns_regexplain_gadget()` in the R console:

`r dspatterns::code_hints(
"Opening up the *RegExplain* gadget is great for experimenting with regular expressions."
)`
```{r open-regexplain, eval=FALSE}
regexplain_gadget()
```

By default, the function provides sample text to match on, but we can provide our own text string as well. The following figure provides a visual on how the gadget looks and it points out its main features.

![The *RegExplain* Gadget as it appears in the **RStudio** Viewer. By interactively typing our regex into the field labeled `RegEx`, we can see what it matches on in the large text field below.](img/regexplain-gadget.png)
...
The dot character (otherwise known as a period, or `.`) is a special token in regex patterns. It matches every character, and that includes spaces as well. Let's look at a few ways we can use the dot with the regex patterns `"...."`, `"....s"`, and `"o."`.

![Using regex patterns with the dot, which unconditionally matches every character. In every case here, the first match is shown.](img/regex-examples-02.png)

We can see from the above that the dot acts as a wildcard, matching every character. Four dots as the pattern, `"...."`, matches the first word (`"The"`) and the following space character. The pattern `"....s"` needs four of any character and then an `s` character (this matches `"jumps"`). Finally, the pattern `"o."` will make its first match on any first appearance of an `o` and any character. In the example sentence, this matches the `ow` in `"brown"`.
...
The next few examples can show us the difference between using the patterns `".."`, `".\\."`, and `"\\.\\."` with our sample sentence.

![Using regex patterns with the escaped form of the dot results in matches involving the period character.](img/regex-examples-03.png)

The first pattern isn't too surprising considering what we now know about the dot character's supreme matching abilities. The second pattern, `".\\.,"` uses the dot (matching all) followed by its escaped form (matching a period); this will match the last two characters in the sentence (that ends with a period). The last pattern, `"\\.\\.,"` just serves to remind us that matching on two literal period characters won't result in a match at all since there's only one period in the entire sentence (a trick question! but educational nonetheless).
...
`r dspatterns::code_hints(
"Using ~~str_match()~~ to determine the first match of the regex pattern on the text.",
c(
"L.1 This is the text string.",
"L.3 The ~~str_match()~~ function will return the first match of ~~\"....\"~~ from the ~~fox_dog~~ string."
))`
```{r str-match}
...
`r dspatterns::code_hints(
"Using ~~str_match_all()~~ to get all of the matches from the text.",
c(
"L.1 The ~~str_match_all()~~ function will return all matches of ~~\".o.\"~~ from the ~~fox_dog~~ string."
))`
```{r str-match-all}
...
The next figure presents a number of regex examples that all use a short phrase concerning quantities of fruit. It combines a number of new things we learned about character classes.

![Using regex patterns with character classes to match on classes of characters.](img/regex-examples-05.png)
...
`r dspatterns::code_hints(
"A regex example that uses character sets, character classes, and negation.",
c(
"L.1 This is the text string.",
"L.3 The ~~str_match_all()~~ function will return all matches the complex pattern from the ~~ihavefruit~~ string."
))`
...
`r dspatterns::code_hints(
"Using the ~~?~~ quantifier to denote an optional piece of text.",
c(
"L.1 This regex pattern gets the fruit names whether they are three or four letters long, and, whether they have singular or plural forms."
))`
...
The following figure provides a few examples of how repetition with `*`, `*?`, and `{n}` is used with the 'A bit of butter makes better batter' phrase.

![The difference between `*` and `*?` during repetition means the difference between greedy and lazy matching. Repetition with `\\w{5}` is exact, matching 5 word characters.](img/regex-examples-06.png)
...
`r dspatterns::code_hints(
"Using anchors to bind the search pattern to the start and/or end of the string.",
c(
"L.1 This regex is anchored to the start of the string with ~~^~~. After the boundary, it wants to match as many word characters as possible.",
"L.2 This regex is anchored to the end of the string with ~~$~~. Before the boundary, it wants to match as many word characters as possible, and then a period before the end of the string.",
"L.3 This here regex is a bit different. It wants to match the entire line (as evidenced by the use of ~~^~~ and ~~$~~. As long as the string is entirely composed of printable characters (it is), a match will occur."
))`
```{r anchors}
...
`r dspatterns::code_hints(
"Using alternation inside of parentheses (which is a capturing group).",
c(
"L.1 This regex is entirely in parentheses and it'll match on the first instance of ~~figs?~~, ~~plums?~~, or ~~limes?~~ (recall that the ~~?~~ makes the preceding token optional). Because it's all in parentheses, this is a capturing group (by default)."
))`
...
`r dspatterns::code_hints(
"Backreferences are used to compose a new string in a particular order with the captured text.",
c(
"L.2 The ~~str_replace_all()~~ function requires both a regex pattern and a replacement string. Here we are using capturing groups for each of the fruits, then, we are composing a new string with the backreferences (in a reverse order).",
"L.6 This is another ~~str_replace_all()~~ call that modifies the output of the previous call. The regex pattern ~~s\\\\b~~ matches on an ~~s~~ character and a word boundary. The replacement of ~~\"\"~~ (an empty string) essentially means that anything matching should be removed.",
"L.7 The **stringr** ~~str_split()~~ function will take a string and split it into a vector using a pattern.",
"L.8 The ~~str_split()~~ function always returns a list. To make this a single vector we can use the ~~unlist()~~ function."
))`
```{r str-replace-all}
ihavefruit |>
  str_replace_all(
...
`r dspatterns::code_hints(
"Extracting words from a string and putting them into a vector with ~~str_extract_all()~~.",
c(
"L.2 The ~~str_extract_all()~~ function just needs a regex pattern and dutifully extract each match from the string and put it into a list.",
"L.3 Just like ~~str_split()~~, the ~~str_extract_all()~~ function will create a list. Because we want a vector we can use the ~~unlist()~~ function."
))`
```{r str-extract-all}
ihavefruit |>
...
`r dspatterns::code_hints(
"Improving the pattern with the word boundary metacharacter.",
c(
"L.4 The use of the word boundary metacharacters ensures that we don't unintentionally match on fragments of words."
))`
```{r str-extract-all-2}
...
`r dspatterns::code_hints(
"Using the ~~str_detect()~~ function to see whether there are any matches of our pattern in the ~~resto_reviews~~ vector.",
c(
"L.1 The ~~str_detect()~~ function, like all <code>str_*()</code> functions in the **stringr** package, always takes the vector of strings as the first argument.",
"L.2 The base R ~~table()~~ function is useful for converting a vector to a contingency table, which is a table counts for each categorical variable (~~TRUE~~ or ~~FALSE~~ in this case)."
))`
```{r str-detect}
...
`r dspatterns::code_hints(
"Using the logical vector from ~~str_detect()~~ to inspect those reviews that matched the regex.",
c(
"L.1 The output of the ~~str_detect()~~ statement is, this time, saved to an object called ~~matches~~.",
"L.3 The logical vector of ~~matches~~ is a great way to subset ~~resto_reviews~~ to those elements where a match occurred."
))`
...
`r dspatterns::code_hints(
"Getting all matches of dollar values and eventually resolving to a single numeric vector.",
c(
"L.1 The pattern ~~\\\\$\\\\d*~~ will match on text like ~~$10~~, ~~$5~~; the resulting object is a list containing all of the matches for each of the vector elements.",
"L.2 Because we want all of the matches in a single vector we use ~~unlist()~~, which will flatten the list into a single character vector.",
"L.3 Eventually, the values should be converted into a numeric vector, however, we can't do that if the dollar sign is present. It's removed by using ~~str_remove()~~, where the pattern is the text that's to be removed.",
"L.4 The character vector can be converted to numeric at this point by using the ~~as.numeric()~~ function."
))`
