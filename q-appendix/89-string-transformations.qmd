...
![The *RegExplain* Gadget as it appears in the **RStudio** Viewer. By interactively typing our regex into the field labeled `RegEx`, we can see what it matches on in the large text field below.](appendix-figures/89-string-transformations/01-figure-regexplain_gadget.png){#fig:regexplain-gadget}
...
![Matching a simple regex pattern to the text and getting the first match.](appendix-figures/89-string-transformations/02-figure-regex_examples_1.png){#fig:regex-examples-1}
...
![Using regex patterns with the dot, which unconditionally matches every character. In every case here, the first match is shown.](appendix-figures/89-string-transformations/03-figure-regex_examples_2.png){#fig:regex-examples-2}
...
![Using regex patterns with the escaped form of the dot results in matches involving the period character.](appendix-figures/89-string-transformations/04-figure-regex_examples_3.png){#fig:regex-examples-3}
...
![Using regex patterns with character sets to match similar words. The pattern in the first example will match two words (even though only the first match is shown).](appendix-figures/89-string-transformations/05-figure-regex_examples_4.png){#fig:regex-examples-4}
...
![Using regex patterns with character classes to match on classes of characters.](appendix-figures/89-string-transformations/06-figure-regex_examples_5.png){#fig:regex-examples-5}
...
![The difference between `*` and `*?` during repetition means the difference between greedy and lazy matching. Repetition with `\\w{5}` is exact, matching 5 word characters.](appendix-figures/89-string-transformations/07-figure-regex_examples_6.png){#fig:regex-examples-6}
