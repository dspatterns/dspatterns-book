...
![The *RegExplain* Gadget as it appears in the **RStudio** Viewer. By interactively typing our regex into the field labeled `RegEx`, we can see what it matches on in the large text field below.](img/regexplain-gadget.png){#fig:regexplain-gadget}
...
![Matching a simple regex pattern to the text and getting the first match.](img/regex-examples-01.png){#fig:regex-examples-1}
...
![Using regex patterns with the dot, which unconditionally matches every character. In every case here, the first match is shown.](img/regex-examples-02.png){#fig:regex-examples-2}
...
![Using regex patterns with the escaped form of the dot results in matches involving the period character.](img/regex-examples-03.png){#fig:regex-examples-3}
...
![Using regex patterns with character sets to match similar words. The pattern in the first example will match two words (even though only the first match is shown).](img/regex-examples-04.png){#fig:regex-examples-4}
...
![Using regex patterns with character classes to match on classes of characters.](img/regex-examples-05.png){#fig:regex-examples-5}
...
![The difference between `*` and `*?` during repetition means the difference between greedy and lazy matching. Repetition with `\\w{5}` is exact, matching 5 word characters.](img/regex-examples-06.png){#fig:regex-examples-6}
