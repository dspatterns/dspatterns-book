...
* The second bit `filters` our `babynames` to only keep rows where the `name` is equal to "Alison" (read `==` as _"exactly equal to"_) *or* "Allison" (read `|` as _"or"_.) 
