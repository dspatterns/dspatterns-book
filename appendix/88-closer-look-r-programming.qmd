...
![](../appendix-figures/88-closer-look-r-programming/01-figure-multiplying_by_a_single_number.png){#fig:multiplying-by-a-single-number}
...
![](../appendix-figures/88-closer-look-r-programming/02-figure-multiplying_by_two_of_same_length.png){#fig:multiplying-by-two-of-same-length}
...
![](../appendix-figures/88-closer-look-r-programming/03-figure-multiplying_by_different_lengths.png){#fig:multiplying-by-different-lengths}
...
c(
"L.2 Piping to an expression inside ~~{ }~~ is a way to use comparison operators with the extracted vector. The dot (~~.~~) represents the extracted vector in the expression.",
"L.3 The ~~any()~~ function takes a logical vector (i.e., a vector of ~~TRUE~~/~~FALSE~~ values) and returns ~~TRUE~~ if there is at least one ~~TRUE~~ value in that logical vector."
))`
```{r}
...
"L.2 This is rightward assignment and, while pretty useful at the end of a piped statement, results in the lack of a ‘subject' (the assigned name) in written code.",
...
![Two approaches to an analysis involving repetitive data. The first doesn't use a generalized function but have to produce a similar analysis script for each dataset. The second approach uses a single function within an **R Markdown** document so that the results can be rendered to a document that can be shared.](appendix-figures/88-closer-look-r-programming/04-figure-reason_why_to_write_functions.png){#fig:reason-why-to-write-functions}
...
![The main parts of an **R** function are the name, the arguments, and the body. The statements typically involve values provided by the arguments.](appendix-figures/88-closer-look-r-programming/05-figure-parts_of_a_function.png){#fig:parts-of-a-function}
