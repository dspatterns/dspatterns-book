...
\
\
`.qmd` files are designed to be used interactively in RStudio. You can use RStudio to test your code one code cell at a time, which we'll show now.
:::
...
::: column
For every gray code cell, you'll notice a green arrow on the right: ![](img/run-button.png)


When you click "Run Current Chunk", the code is sent straight to your console. 

If you run the second code cell after the first, it also works! This is because RStudio is running your code cells in your global environment, so it finds the `words` dataframe.
...
1. We reorganized our code cells so that all packages are loaded in the first code cell, and
...
In this section, we showed how you can develop Quarto documents in RStudio, one code cell at a time. A `.qmd` file can be used to both save and execute code, and to create a reproducible record of what you did and how you did it. How to "knit" this record is up next.
...
-   A code cell (but we only see one...)
...
See the [R Markdown Reference Guide](https://www.rstudio.com/wp-content/uploads/2015/03/rmarkdown-reference.pdf) for a complete list of **knitr** options.

Style your figures w/ knitr options (out.width, fig.show, fig.align, fig.asp, fig.cap, fig.alt)
...
By contrast, the code cell on the right will create a plot object named `line`.
